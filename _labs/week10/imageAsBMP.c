...
static unsigned int calculateImageSize (Image i);
...
static size_t allocImageBuffer (
    ...
    // Calculate the image size, taking into account any necessary
    // padding bytes.
    size_t allocSize = calculateImageSize (i);
...
static void writeDIBHeader (
    ...
    // File size
    unsigned int fileSize = calculateImageSize(i);
...
// However, if the image isn't a multiple of 4, we need to
// take into account the padding bytes required.
static unsigned int calculateImageSize (Image i) {
    unsigned int rowSize = imageGetWidth (i) * PIXEL_BYTES;
    unsigned int rowPadding = 0;
    if (rowSize % PIXEL_ALIGN != 0) {
    ...
    unsigned int pixelData = PIXEL_START + rowSize * imageGetHeight (i);
